<!--
GENERATED — TOOLKIT v2.0.0 — bundled from schema/ + style_tokens/. (Version marker: Ctrl+F "TOOLKIT v" in a chat to confirm which build is pasted; bumped on every release, see CHANGELOG.md.)
...
14. **Self-check receipt (§0.6 gate — MANDATORY):** every §4 answer ends with the `# ASSUMPTIONS` block (print it even when nothing was invented — then it contains only the receipt), whose LAST line is:
    `# SELF-CHECK: opener=§2 template ✓ · prose, no bullets ✓ · hex inline per surface ✓ · coverage rule 11 ✓ · entry points deduped ✓ · closed element lists ✓ · word budget ✓ · negatives translated & guarded ✓`
...
3. `STYLE` on refs that show UI ⇒ `color_map` is REQUIRED; REVIEW NOTES opens with the `# CONSISTENCY:` receipt.
4. Every §4 prompt: opener from the §2 template · flowing prose, no bullets · hex INLINE per surface · one entry point per feature · closed element lists · ends with `# ASSUMPTIONS` + the `# SELF-CHECK:` receipt.
