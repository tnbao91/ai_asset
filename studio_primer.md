<!--
GENERATED — TOOLKIT v2.0.3 — bundled from schema/ + style_tokens/. (Version marker: Ctrl+F "TOOLKIT v" in a chat to confirm which build is pasted; bumped on every release, see CHANGELOG.md.)
...
8. **Consistency check (run silently BEFORE printing the YAML — fix violations, don't note them):**
   ...
   - **Negative-contradiction check:** no `negative` entry may contradict a field you filled — a dark background/palette forbids `dark_theme` in the list; `outline.enabled: true` forbids any lineart-banning entry; rendering `fully_painted`/`realistic_painted` forbids banning brushstrokes/realistic; a textured rendering forbids `noisy_texture`. On conflict, drop the negative entry (the filled field wins — it IS the ref's style).
   ...
   - **Shape re-measure (LOOK at the ref again before printing `shape` + `typography.font_feel` — do not answer from habit):** (a) are any panels / tabs / banners slanted or skewed (parallelogram edges, non-vertical sides)? → `shape.slant` MUST be filled (slight/strong); (b) is the display lettering italic / forward-leaning? → `font_feel: italic_display`, not bold_display; (c) is `corner_radius` a measured read of the ref (crisp arcade cards = `small`) rather than the habitual `medium`? Emitting rounded-upright-medium without having re-looked is the single most common analyzer bug. A straight, upright, medium-radius ref is a perfectly valid answer — but only after looking.
   Then OPEN the `# REVIEW NOTES` block with one receipt line proving each check ran — `# CONSISTENCY: single-value fields ✓ · negatives-vs-filled-fields ✓ (none conflict) · color_map filled (refs show UI) / omitted (no UI) · confidence↔blocks paired ✓ · shape re-measured: slant=<filled/none — why> · lettering=<italic/upright> · corners=<measured value>` (§0.6 gate). The three `shape re-measured` slots must state what you SAW, not repeat a template. A style_guide printed without this line is invalid — go back and run the checks.
...
14. **Self-check receipt (§0.6 gate — MANDATORY):** every §4 answer ends with the `# ASSUMPTIONS` block (print it even when nothing was invented — then it contains only the receipt), whose LAST line is:
    `# SELF-CHECK: opener=§2 template ✓ · prose, no bullets ✓ · hex inline per surface ✓ · coverage rule 11 ✓ · entry points deduped ✓ · closed element lists ✓ · aspect ratio sourced (rule 5) ✓ · word budget ✓ · negatives translated & guarded ✓`
...
4. Every §4 prompt: opener from the §2 template · flowing prose, no bullets · hex INLINE per surface · one entry point per feature · closed element lists · aspect ratio from spec/ref/`canvas` — never a blind 9:16 · ends with `# ASSUMPTIONS` + the `# SELF-CHECK:` receipt.
